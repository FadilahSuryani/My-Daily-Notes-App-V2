...
function Home() {
  ...
  function onLogoutHandler(event) {
    ...
    const isConfirmed = window.confirm("are you sure log out of account?");
    if (isConfirmed) {
      deleteAccessToken();
      navigate("/");
    }
  ...
  return (
    <>
      ...
      <div className="p-5 px-3">
        <strong className="p-5 fs-1 text-dark">Note List</strong>

        <br />
        <Button
          className="d-flex mb-2 btn-outline-warning text-center"
          variant="dark"
          type="submit"
          onClick={() => {
